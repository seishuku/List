...
// Care must be taken when using List_Init or List_Add, data being passed *must* coinside
//		with stride/count used when initializing the list.
// As well with List_GetCopy data argument must be a valid pointer to an approperate size memory
//		location for the single item of "stride" size.
...
bool List_ShrinkFit(List_t *list)
{
	...
	uint8_t *temp=(uint8_t *)realloc(list->buffer, list->size);
